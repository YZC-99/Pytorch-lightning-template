...
class LockEncoderUNet(UNet):
    def __init__(self,
        ...
        super(LockEncoderUNet, self).__init__(
                 image_key,
                 in_channels,
                 num_classes,
                 bilinear,
...
class probUNet(UNet):
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        x_dict = self.encoder(x)
        decoder_out = self.decoder(x_dict)
        logits = self.out_conv(decoder_out['x_up4'])

        # prob, max_indices = torch.max(logits, dim=1)
        # od_prob = prob * label_od
        # oc_pseudo_label = torch.zeros_like(od_prob)
        # oc_pseudo_label[od_prob == 1] = 1
