...
class ClassificationBase(Dataset):
    ...
    def __getitem__(self, i):
        ...
        letter = os.path.basename(example["file_path_"]).split('_')[0]
        if letter == 'NL':
            class_label = 0
        else:
            class_label = 1
...
class Multi_diseaseTrain(ClassificationBase):
    def __init__(self, size=None, train=True, interpolation="bicubic",
                 data_csv='data/multi_disease/train.txt',
                 data_root='data/multi_disease/',
                 ):
...
class Multi_diseaseEval(ClassificationBase):
    def __init__(self,
                 size=None,
                 train=False,
